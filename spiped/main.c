...
main(int argc, char * argv[])
{
	...
	if (graceful_shutdown_initialize()) {
		warn0("Failed to initialize graceful shutdown");
		goto err1;
	}
	...
	/* Start the graceful_shutdown timer to monitor the shutdown signal. */
	if (graceful_shutdown_monitor(&callback_graceful_shutdown,
	    dispatch_cookie)) {
